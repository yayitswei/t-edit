...
(defn render-elem [{:keys [type color color-preview x y height width font font-preview font-size d scale]
               :as elem}]
  (case type
    :text
    [:text {:fill (or color-preview color)
            :stroke (or (:outline-color elem) color-preview color)
            :x x
            :y y
            :text-anchor "middle"
    ...
    [:path {:fill (or color-preview color)
            ...
            :width width
            :height height
            :transform (apply transform-str (transform-matrix {:x x :y y :scale-x scale :scale-y scale}))}]
...
(defn elem-color-selection [color]
  [:li {:key color
        :class "color-selection elem-color-selection"
        :style {:background color}
        :on-mouse-over (fn [_]
                         (when @selected
...
(defn elem-color-selector []
  [:div {:class "form-group"}
   [:div {:class "btn-group dropup"}
    [:button {:type "button"
              :class "btn btn-default btn-lg dropdown-toggle"
              :data-toggle "dropdown"}
     [:div {:id "elem-color-selector-label"}
    ...
    [:ul {:class "dropdown-menu color-selector elem-color-selector" :role "menu"}
     (for [c (:elem-colors config)] [elem-color-selection c])]]])
...
(defn init-events! []
  ...
  (js/window.addEventListener "keydown" (fn [e] (when (= 13 (.-keyCode e))
                                                  (.preventDefault e)
